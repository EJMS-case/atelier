...
6. **Hosiery images**: 15 of the 21 pairs now use real bg-removed product photos — black sheer/semi-opaque and skin-tone sheer from the Noosh order #457985 receipt email, the rest matched per-variant from the store's product JSON (Midnight = navy, Berry = burgundy, Brew = brew, Espresso = brown; sheer/semi-opaque/opaque each have their own shade galleries). PNGs live in `scripts/assets/hosiery/`, applied via `scripts/apply-noosh-photos.mjs` (idempotent). The remaining 6 pairs keep generated SVG placeholders: skin-tone semi/opaque, brown semi-opaque (Noosh never made those variants) and the 3 micro-fishnet rows.
