...
6. **Hosiery images**: 7 of the 21 pairs now use real bg-removed product photos — black sheer, black semi-opaque, and skin-tone sheer from the Noosh order #457985 receipt email, plus opaque navy/burgundy/brew/brown from Noosh's product galleries (shades Midnight/Berry/Brew/Espresso). PNGs live in `scripts/assets/hosiery/`, applied via `scripts/apply-noosh-photos.mjs` (idempotent). The remaining 14 pairs keep generated SVG placeholders (`scripts/seed-hosiery.mjs`) — Noosh only makes the colored shades as opaque, so no real imagery exists for e.g. sheer navy.
