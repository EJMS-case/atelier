...
6. **Hosiery images**: 3 of the 21 pairs now use real bg-removed product photos pulled from the Noosh order #457985 receipt email — black sheer, black semi-opaque, skin-tone sheer ("Dulce"); PNGs live in `scripts/assets/hosiery/`, applied via `scripts/apply-noosh-photos.mjs` (idempotent). The other 18 colorway/opacity pairs still use generated SVG placeholders (`scripts/seed-hosiery.mjs`) — no receipt imagery exists for them.
