...
function filterByWeather(items, weather) {
  ...
  return items.filter(it => {
    ...
    if (isHot) {
      ...
      if (isKnitDress) return false;  // knit/sweater dresses are not hot-weather appropriate
      if (it.subcategory === "Sweater Dress") return false;  // subcategory-based catch
      if (it.subcategory === "Boots") return false;
      if (it.subcategory === "Coats") return false;
      if (it.subcategory === "Jackets" && isHeavyFabric) return false;  // allow lightweight jackets
      // Allow blazers through unless heavy (wool blazers etc. caught by isHeavyFabric below)
      if (it.category === "Tops" && sleeve === "long") return false;
      // Filter long-sleeve dresses in hot weather too
...
function colorSortIdx(item) {
  ...
  // Partial match — check if color contains a known key
  for (const [key, idx] of Object.entries(COLOR_SORT_ORDER)) {
    if (c.includes(key.toLowerCase())) return idx;
  }
  // Keyword fallback for common color words
...
// ── STYLING STRATEGIES — rotated randomly to force genuine variety ────────────
// These are concrete creative directions (not named archetypes) that push the AI
// to take fundamentally different approaches for each look.
const STYLING_STRATEGIES = {
  // COLOR APPROACHES — how the palette is built
  color: [
  ...
  // PROPORTION APPROACHES — how the silhouette is constructed
  proportion: [
  ...
  // HERO PIECE TYPES — what anchors the outfit
  hero: [
...
async function generateOutfit(items, occasion, weather, request, apiKey, previousLooks = [], stylePrefs = STYLE_PREFS, aboutMe = {}, styleExcludes = new Set()) {
  ...
  // In hot/warm weather, downgrade layer from required to optional
  // so the AI isn't forced to include blazers/coats in sweltering heat
  const slots = (() => {
  ...
  // ── STEP 2: Normalize About Me — use null for unset fields ──
  ...
  // ── STEP 3: Build active exclusions as human-readable labels ──
  ...
  // ── STEP 6: Format inventory for the prompt ──
  const inventory = formatInventory(sampled, getSleeveType);

  // ── STEP 7: Build availability note ──
  ...
  // ── STEP 8a: Select 3 random styling strategies for variety ──
  // Each look gets a different color approach, proportion, and hero type
  // This forces fundamentally different creative directions without named archetypes
  ...
  // ── STEP 8b: Build the full prompt using the new template ──
