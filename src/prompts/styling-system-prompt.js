...
- HC_SHOULDER Work and Work Dinner only, in EVERY weather — her office is business professional. A long-sleeve top (sleeve tag [L] or [3Q]), a long-sleeved dress, or a turtleneck stands on its own — DO NOT add a layer over a long sleeve just to "be safe." A short-sleeve top ([S]), a tank or anything sleeveless ([N]: tank, strappy, halter, off-shoulder, strapless, slip dress) takes a Knits or Outerwear layer over it, worn open. In WARM or HOT weather the layer changes, not the rule: reach for the LIGHTEST layer in the inventory — a fine-gauge cardigan, an unlined or linen blazer — never a wool coat or heavy blazer, and never no layer over a short-sleeve or sleeveless top.
...
// Bucket tests go through weatherMatches (constants/taxonomy) rather than
// inline regexes. WEATHER_BUCKETS is the one definition of what "Warm" means,
// and a copy of it here is a copy that can drift: #218 swept exactly these
