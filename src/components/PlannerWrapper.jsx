...
export default function PlannerWrapper({ items, onGoToStyleMe, onEditItem }) {
  const [logs, setLogs] = useState([]);
  useEffect(() => {
    sb.fetchOutfitLogs().then(setLogs).catch(() => {});
  }, []);
  return <CalendarView items={items} outfitLogs={logs} onGoToStyleMe={onGoToStyleMe} onEditItem={onEditItem}/>;
}
