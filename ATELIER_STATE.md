...
- **Most worn**, **cost per wear**, and **Back in Rotation**: pieces resting 60+ days, restricted to restyle-worthy garments, season-filtered by the live forecast, rotated daily. Tapping a piece styles it.
...
- **Item fields**: name, category, subcategory, brand, colour (with a derived colour family), material, pattern, formality (1–8), knit weight and fit, notes, **stylist line** (a curated ≤200-character line that is what the AI reads), price paid, set membership and separability, closet, plus photo flags and the vision read.
...
- **Edit item**: every field, the stylist line, knit weight with the phrase the app read from her notes when the tag is empty, the piece's wear history (dated outfits it appeared in, plus upcoming planner pins), photo replacement with background removal.
...
Wear stats are derived from the record itself (calendar days plus dated logs), counting distinct days per piece, never from a stored counter that could drift. That feeds most-worn, neglected (60+ days), cost per wear when a price is set, the per-item history inside Edit, the Home recap, and the stylist's occasion memory (the pieces she returns to per occasion over the last 180 days, loved looks counted double).
...
- **Stylist line**: the curated ≤200-character description of a piece that every classifier and prompt reads; her notes are the fallback.
