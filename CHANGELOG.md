...
## [Unreleased] — The gap analysis shops at her price, from her wardrobe, and checks every pick against her closet before she sees it — 2026-09-17
...
### Why
Owner, from her phone: *"I am editing a saved outfit"* — with a screenshot of the
maroon-blazer look, and a second screenshot of what tapping **Edit** gave her: a
blank canvas, every slot chip reading `+`, nothing to pick from.

`SilhouetteBuilder` reads its pool from a prop named `items`. The pool-vocabulary
rename (#217) renamed that prop **at the call site and not in the component** —
`LooksView` and `OutfitHistory` both passed `wardrobe={…}`. React does not check
prop names, so the component ran with its pool `undefined`: the look's ids
resolved against nothing, every picker list filtered nothing, and the screen
rendered perfectly empty. Build green, 32 suites green, the render walk green —
it never opened the builder.

Every path into the builder from **Saved** was affected, not just Edit: "Build a
Look" and History → Edit opened the same empty screen. Style Me's builder was
fine (App.jsx passed the prop it declared), which is why this looked like a
Saved-tab problem rather than a builder problem.

### Fixed
- **The builder's pool prop is now `builderPool`**, the name the vocabulary
  already gives it (an `available` widened for one surface), documented in the
  signature — and all three call sites hand it that.
- **`LooksView` and `OutfitHistory` now build the pool properly**, with
  `poolIncluding(available, wardrobe, <the ids of the look being edited>)` —
  the same pool `App.jsx` builds for Style Me. Both halves matter: the widening
  keeps an out-of-closet piece on the canvas (and stops Save writing the thinned
  look back), and the `available` base stops the swap sheet offering her pieces
  that are 2,000 miles away. `available` and `setsMeta` are threaded through
  `SavedView` to reach them.
- `StyleInsightsView` declared a `wardrobe` prop that nothing passed and nothing
  read — dropped.

### Added
- **`npm run test:props` — the prop contract.** Statically pairs every JSX call
  site of a local component against that component's declared props, both ways:
  a prop passed that the component never reads (this bug), and a
  no-default prop that no call site ever passes (its mirror image — it found the
  dead `wardrobe` above). 64 call sites, 46 components. It fails on any call site
  it cannot parse rather than skipping it, and prints its own blind spot — the
  components taking props whole or with a rest, and the call sites using
  `{…spread}` — so the coverage stays visible.
- **Two render-walk steps that actually open the builder**: Edit must land the
  look in its slots and on the canvas, and the picker must offer the wardrobe
  including the look's own out-of-closet piece. The walk is 11 screens.
- A runtime guard in `SilhouetteBuilder`: no pool → a console error, which the
  render walk fails on.

### Notes
- **Validated by reintroducing the bug**, per the standing rule: with
  `wardrobe={…}` put back, `npm run build` passes, all 33 unit suites pass, and
  all three new guards go red — the prop contract names the file and line, and
  both walk steps fail with what she saw.
- Nothing was lost from her saved looks. Save is disabled below two pieces, so
  the empty builder could not write an empty look back over the one she opened.
...
### Added
- **`src/features/closet/poolInvariants.js`** — the rules the app has always assumed and never checked, as pure functions, so the same rule runs in unit tests AND against live data. The one that matters: *a committed set of ids must be resolvable in the pool it is shown and edited from.* Closet scoping answers "what may I PICK here"; it must never decide what an already-committed thing CONTAINS. `unreachableIds` / `classifyUnreachable` (which distinguishes a piece the pool HIDES from one the wardrobe genuinely lost — conflating those two is how every bug this week stayed invisible), `looksReachable`, `setsSplitAcrossClosets`, `activeTripCarriesSomething`, `taxonomyAnomalies`, `miscLeaks`.

  `setsSplitAcrossClosets` is deliberately **duplicate-aware**: a set living in two rooms is normal here, so it reports only a cross-room set where nothing links the halves — no piece on either side is a copy of a piece on the other. Over her live data that is exactly one set (a Good Karma bra filed with two Never Better pieces), and the seven duplicate pairs stay silent.
...
### Changed
- **Coord-set membership is now closet-scoped on purpose, in one shared place.** The Coord Set panel and the set editor each carried their own inline `filter(it => it.set_id === …)` over whatever array they were handed. Both now call `setMembers()` / `setMatesOf()` in `setType.js`, which scope by the tapped piece's own closet and take the FULL wardrobe — so the answer no longer depends on the caller passing the right array.

  **Corrected on the owner's review, and worth recording.** The first version of this resolved membership across *all* rooms, on the theory that a set spanning two closets was the same bug as a look losing its out-of-closet pieces. She caught it: *"For coord sets — I have the same set in both closets. Make sure your change didn't change that."* She is right, and the data agrees. She buys athleisure in twos and the ⧉ duplicate feature copies a piece into the other room **keeping its `set_id`**, so seven of her eight cross-closet sets are one set owned twice, with the Arizona rows carrying `duplicate_of` back to their NYC twins. Resolving across rooms would have turned a 2-piece set into a 4-piece one, listing each garment twice, half of them 2,000 miles away. The closet scoping was never the bug — it was the feature.


### Notes
Running the doctor over her live 533-garment wardrobe found three things, all left for her: one of the Arizona trip's 18 pins names a garment that has since been deleted (a dangling suitcase entry, so the packer reserves a slot for something gone); `Sports Bra` (1 row) vs `Sports Bras` (19 rows) is one idea filed two ways, so any code comparing that subcategory covers only half the rows; and one coord set groups a Good Karma bra with two Never Better pieces across the two closets, which looks like a mis-file rather than a pair. No migration, no data written.
